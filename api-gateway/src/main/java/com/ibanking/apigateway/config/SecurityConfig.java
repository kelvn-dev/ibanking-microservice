...
import com.ibanking.apigateway.config.properties.Oauth2PropConfig;
...
public class SecurityConfig {
  ...
  SecurityWebFilterChain springSecurityFilterChain(ServerHttpSecurity http) throws Exception {
    ...
    if (oauth2PropConfig.isDisabled()) {
      ...
      http.authorizeExchange(
              authorize ->
                  authorize.pathMatchers(byPassPaths).permitAll().anyExchange().authenticated())
  ...
  public ReactiveJwtDecoder jwtDecoder() {
    final NimbusReactiveJwtDecoder decoder =
        NimbusReactiveJwtDecoder.withJwkSetUri(oauth2PropConfig.getJwkSetUri()).build();
    ...
    final List<OAuth2TokenValidator<Jwt>> validators =
        List.of(
            ...
            new JwtClaimValidator<List<String>>(
                ...
                aud -> aud.contains(oauth2PropConfig.getAudiences())));
